	...
	public function addComplexType($type, $name = false, $parent = false)
	{
		...
		if(strtolower(substr($type,0,6)) == 'array(' || substr($type,-2) == '[]'){
			...
			if(strtolower(substr($type,0,6)) == 'object'){//stdClass
				...
				if(!class_exists($name)) throw new customException("Error creating WSDL: no class found with the name '$name' / $type : $parent, so how should we know the structure for this datatype?");
				$v = new genreflectionClass($name);
