...
$url = $this->uri($linkarray, 'courselecturers');
// Create a table
...
foreach ($allUsers as $user) {
    ...
    if ($user['userid'] == $objUser->userId()) {    	
        ...
        $makeBuddy = "<a href = \"" . $this->uri(array('module' => 'courselecturers',
                'action' => 'AddBudy',
                'buddyId' => $user["userid"],
                'how' => 'firstname',
                'searchField' => $this->getParam('searchField')
                ))
         . "\">" . $objLanguage->languageText('mod_courselecturers_makebuddy', 'courselecturers') . "</a>";
    } else {
        $makeBuddyIcon = $this->getObject('geticon', 'htmlelements');
        $makeBuddyIcon->setIcon('user_user');
        $makeBuddyIcon->alt = $objLanguage->languageText('mod_courselecturers_isalecturer', 'courselecturers');
...
$objBackLink->link = $objLanguage->languageText('mod_courselecturers_return', 'courselecturers'); //mod_buddies_return
...
if (empty($allUsers)) {
    $middleColumnContent .= "<p>" . "<span class='noRecordsMessage'>" . $objLanguage->languageText('mod_courselecturers_nouser', 'courselecturers') . "&nbsp;" . '"' . $searchField . '"' . "</span>" . "</p>";
} 
