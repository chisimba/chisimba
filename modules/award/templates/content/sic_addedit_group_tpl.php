...
if(isset($sicGroupId))
{
	$objAddEditGroupForm = new form('lrssic', $this->uri(array('action'=>'editgroup', 'sicGroupId'=>$sicGroupId, 'selected'=>'init_10')));
}
else
{
	$objAddEditGroupForm = new form('lrssic', $this->uri(array('action'=>'addgroup', 'selected'=>'init_10')));
}
...
$header->type = 2;
if (isset($sicGroupId)){
	$header->str = $this->objLanguage->languageText('mod_lrssic_edit_group', 'award');
	$objAddEditGroupForm->addToForm($header->show());
}
else 
{
	$header->str = $this->objLanguage->languageText('mod_lrssic_add_group', 'award');
	$objAddEditGroupForm->addToForm($header->show());
}
...
$objaddeditHeadTable->addCell("<i>".$this->objLanguage->languageText('mod_lrssic_tbl_head_group', 'award')."</i>");
...
if(isset($sicGroupId))
{
	$valueRow = $this->objDbSicGroups->getRow('id', $sicGroupId);
...
$location = $this->uri(array('action'=>'selectsicgroup', 'sicMajorGroupId'=>$sicMajorGroupId, 'selected'=>'init_10'), 'award');
...
$objAddEditGroupForm->addRule('description',$this->objLanguage->languageText('mod_lrssic_desc_rule_majordiv','award'),'required');
$objAddEditGroupForm->addRule('code',$this->objLanguage->languageText('mod_lrssic_code_rule_majordiv','award'),'required');
$objAddEditGroupForm->addToForm($objaddeditHeadTable->show());
$objAddEditGroupForm->addToForm($objaddeditTable->show());

echo $objAddEditGroupForm->show();
?>