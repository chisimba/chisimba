...
$objHeading->type = 1;
$objHeading->str = $objUser->getSurname() . $objLanguage->languageText("mod_eportfolio_competencylist", 'eportfolio') . '&nbsp;&nbsp;&nbsp;' . $linkAdd;
echo $objHeading->show();
$list = $this->objDbCompetencyList->getByItem($userId);
echo "<br/>";
...
$table->addHeaderCell("<b>" . $objLanguage->languageText("mod_eportfolio_qclawarddate", 'eportfolio') . "</b>");
...
if (!empty($list)) {
    foreach($list as $item) {
        ...
        $table->addCell($this->objDate->formatDate($item['award_date']) , "", NULL, NULL, $class, '');
