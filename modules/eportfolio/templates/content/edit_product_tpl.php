...
$objHeading->type = 1;
$objHeading->str = $objLanguage->languageText("mod_eportfolio_editProduct", 'eportfolio');
...
$objTable->addRow($row, NULL);
//type text box
$textinput = new textinput("producttype", $producttype);
$textinput->size = 40;
$row = array(
...
$objTable->addRow($row, NULL);
//comment text box
$textinput = new textinput("comment", $comment);
$textinput->size = 40;
$row = array(
...
$textinput = new textinput("referential_source", $referential_source);
$textinput->size = 40;
$row = array(
...
$textinput = new textinput("referential_id", $referential_id);
$textinput->size = 40;
$row = array(
    ...
    foreach($Id as $groupId) {
        ...
        foreach($parentId as $myparentId) {
            ...
            if (!empty($list)) {
                ...
                $dropdown->setSelected($assertion_id);
            }
        }
    }
} else {
...
if (!empty($userassignments)) {
    ...
    foreach($userassignments as $myassignments) {
        ...
        $dropdown->setSelected($assignment_id);
    }
} else {
...
$objTable->addRow($row, NULL);
//essay label
$textinput = new textinput("essay_id", $essay_id);
$textinput->size = 40;
$row = array(
