 ...
 * @category  Chisimba
 * @package   kngimport
 * @author    James Scoble,Yasser Buchana, Jarrett Jordan
...
// end security check.
class remoteimportexportutils extends dbTable
{
	/**
	...
	*/
	 ...
	 */
    	public function setup($server) 
    	{
	      ...
	      $server= $this->objSysConfig->getValue('remote_server','kngimport'); //'localhost'; 
	      $user= $this->objSysConfig->getValue('remote_user','kngimport');//'root';
	      $password= $this->objSysConfig->getValue('remote_password','kngimport');
	      $dbase= $this->objSysConfig->getValue('remote_dbase','kngimport');
	      
	      $dsn='mysql://'.$user.':'.$password.'@'.$server.'/'.$dbase;
	      //var_dump($dsn);
	      return $dsn;
	 ...
	 */
		...
		{
            		throw new customException($this->objLanguage->languageText("mod_kngimport_import_noconn", "kngimport"));
        	}
	...
	*/
	function writeFiles($htmlPages, $resourcecreateFolder, $courseTitle = '', $fileType = '', $packageType = '')
	{
		...
		$courseTitle = str_replace('_','',$courseTitle);
                $htmlFilenames=array(); //codefix by jsc
		$i = 0;
	...
	*/
	function writeSchemaFiles($tempDirectory)
	{
		...
		$schamas = $siterootpath."core_modules/kngimport/ims/";
	...
	*/
	...
	public function importTemplate($dbData, $packageType, $newCourse)
	{
		...
		if($newCourse)
			$objH->str = $this->objLanguage->languageText("mod_kngimport_selectCourse","kngimport");
		else
			$objH->str = $this->objLanguage->languageText("mod_kngimport_selectCourseHeading","kngimport");
		$objH->type=2;
		...
		// Label.
		$label = new label($this->objLanguage->languageText("mod_kngimport_selectCourse","kngimport"),"select");
		// Button.
	...
	public function uploadTemplate($section = '')
	{
		...
		$form1 = new form('uploadziplocal', $this->uri($paramArray1,'kngimport'));
    		$form1->extra=' enctype="multipart/form-data" ';
    		// File input.
    		$fileInput = new textinput('upload');
    		$fileInput->fldType='file';
    		$fileInput->label=$this->objLanguage->languageText("mod_kngimport_uploadNotice","kngimport");
    		$fileInput->size=50;
    		// Submit button.
		...
		$checkLabel = new label($this->objLanguage->languageText("mod_kngimport_createCourse","kngimport"),"");
		// Checkbox.
		...
		$form2 = new form('uploadziplocal', $this->uri($paramArray2,'kngimport'));
    		$form2->extra=' enctype="multipart/form-data" ';
    		// Heading.
		$heading1 = new htmlheading();
    		$heading1->str=$this->objLanguage->languageText("mod_kngimport_uploadserver","kngimport");
    		$heading1->type=2;
		// Button.
		...
		// Label - server.
		$serverLabel = new label($this->objLanguage->languageText("mod_kngimport_selectServer","kngimport"),"");
		...
		// Label - local server.
		$localLabel = new label($this->objLanguage->languageText("mod_kngimport_localServer","kngimport"),"");
    		// Server input.
    		$serverInput = new textinput($this->objLanguage->languageText("mod_kngimport_localhost","kngimport"), $this->objLanguage->languageText("mod_kngimport_uploadInfo","kngimport"),'','45');
		// Label - Create Course.
		$checkLabel2 = new label($this->objLanguage->languageText("mod_kngimport_createCourse","kngimport"),"");
		$form2->addToForm('<hr>');
	...
	public function downloadTemplate()
	{
		...
		$form1 = new form('exportziplocal', $this->uri($paramArray1,'kngimport'));
    		$form1->extra=' enctype="multipart/form-data" ';
		// Label - server.
		$courseLabel = new label($this->objLanguage->languageText("mod_kngimport_selectCourse","kngimport"),"");
