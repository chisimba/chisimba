...
if (!empty($arrMaps)){
    ...
    foreach ($arrMaps as $map){
        ...
        */
        ...
        if ($map['is_dynamic'] == '1') {
            $objIcon->setIcon('checked', 'gif', 'icons/phpunit/');
        } else {
            $objIcon->setIcon('unchecked', 'gif', 'icons/phpunit/');
        }
        ...
        $editIcon = "<a id='edit_$map[id]' title='Edit URL Mapping' href='http://localhost/fresh/?module=phpunit&action=getform&type=addeditform&id=$map[id]'>".$objIcon->show()."</a>";
        //} else {
        //    $editIcon = '';
        //}
