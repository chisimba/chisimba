...
$icon->setIcon($topic['type_icon'], NULL, 'icons/forum/');
$icon->alt= $topic['type_name'];
$icon->title=$topic['type_name'];

$header = new htmlheading();
$header->type=1;
$header->str=$icon->show().' '.$this->objLanguage->languageText('mod_forum_topicstatus').': '.stripslashes($topic['post_title']);
echo $header->show();
...
$forumLink = new link($this->uri(array( 'module'=> 'forum', 'action' => 'forum', 'id' => $topic['forum_id'])));
$forumLink->link = $this->objLanguage->languageText('mod_forum_backtoforum');
$forumLink->title = $this->objLanguage->languageText('mod_forum_backtoforum');
