<?php
/**
 * ahis  species categories admin Template
 *
 *
 * PHP version 5
 * 
...
$addLink = new link($this->uri(array('action'=>'speciescategories_add')));
$addLink->link = 'New species category add';

$objHeading = $this->getObject('htmlheading','htmlelements');
$objHeading->type = 2;
$objHeading->str = 'Species categories admin';
...
$objTable->addHeaderCell($this->objLanguage->languageText('phrase_category'));
...
if (!empty($data)) {
	...
	$link = $icon->getDeleteIconWithConfirm($datum['id'],array('action'=>$deleteAction,'id'=>$datum['id']), 'openaris', $message);
        ...
        $objTable->addCell($datum['category']);
