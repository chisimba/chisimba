<?php
/**
 * ahis Exchange rate Admin Template
 *
 *
 * PHP version 5
 * 
...
$addLink = new link($this->uri(array('action'=>'exchangerate_add')));
$addLink->link = $this->objLanguage->languageText('phrase_addexchangerate');

$objHeading = $this->getObject('htmlheading','htmlelements');
$objHeading->type = 2;
$objHeading->str = 'Exchange rate admin';
...
$objTable->addHeaderCell('Default currency');
$objTable->addHeaderCell('Exchange currency');
$objTable->addHeaderCell('Start date');
$objTable->addHeaderCell('End date');
...
if (!empty($data)) {
    foreach ($data as $datum) {
        ...
        $objTable->addCell($datum['defaultcurrencyid']);
        $objTable->addCell($datum['exchangecurrencyid']);
        $objTable->addCell($datum['startdate']);
		$objTable->addCell($datum['enddate']);
...
$objPagination = $this->newObject('pagination', 'navigation');
    $objPagination->module = 'openaris';
    $objPagination->action = 'getcontextajax';
    $objPagination->id = 'pagenavigation_context';
    
    $itemsPerPage = ($numContextAhis - ($numContextAhis % $this->itemsPerPage)) / $this->itemsPerPage;
    if ($numContextAhis % $this->itemsPerPage != 0) {
        $itemsPerPage++;
    }
