 ...
 * @version    $Id: Trackback.php 11076 2008-10-25 18:13:10Z charlvn $
...
class Services_Trackback
{
    ...
    function send($data = null) 
    {
        ...
        // Add HTTP headers
        $req->addHeader("User-Agent", $this->_options['httprequest']['useragent']);
        // Adding data to send
        ...
        if (isset($this->_data['itemId'])) {
            $req->addPostData('itemId', strip_tags($this->_data['itemId']));
        }
        if (isset($this->_data['trackId'])) {
            $req->addPostData('trackId', strip_tags($this->_data['trackId']));
        }
        if (isset($this->_data['itemName'])) {
            $req->addPostData('itemName', strip_tags($this->_data['itemName']));
        }
        // Send POST request
        $res = $req->sendRequest();
        if (PEAR::isError($res)) {
...
EOD;
    ...
    {
        $data = Services_Trackback::_getEncodedData(array(
            'code',
            'message'
        ) , array(
            'code' => $code,
