 ...
 **/
 ...
 class dbcontentnodes extends dbtable{
    ...
    /**
     * @var object $objDBContext : The context database object
     */
    var $objDBContext;
    
    /**
    * @var array $deleteList : This list of nodes that needs to be deleted
    */
        ...
        switch($mode){
            ...
            case 'add': 
                ...
                $nodeId=$this->insert(array(
                    'tbl_context_parentnodes_id' => $this->objDBContext->getRootNodeId(),
                    'title' => $title,
                    'prev_Node' => $prev_Node,
                    'next_Node' => '',
                    'sortindex' => $sortIndex ,
    ...
    * a context has nodes
    */
    function hasNodes($contextId=NULL)
    {
    ...
    */
    function createParentNode()
    {
        ...
        if(!$this->objDBParents->valueExists('tbl_context_parentnodes_has_tbl_context_tbl_context_contextCode',$this->objDBContext->getContextCode()))
        {
          ...
          // $this->changeTable('tbl_context_parentnodes_has_tbl_context');
            $this->objDBContextParents->insert(array(                
           ...
           // $this->changeTable('tbl_context_parentnodes');
            $parentId=$this->objDBParents->insert(array(
                    ...
                    'userId' => $this->userId,
                    'dateCreated' => $this->objDBContext->getDate(),
                    'datemodified' => $this->objDBContext->getDate(),
                    'menu_text' => $this->objDBContext->getMenuText(),
                    'title' => $this->objDBContext->getTitle()));
        }    
        $this->resetTable();
    }
    ...
    */
    function getSharedNodes($parentNodeId)
    {
    ...
    function shareNodes($sharedNodeId, $currentContextNodeId, $contextId=NULL){
        ...
        if(! array_key_exists(0,$res) ) {
            $newId = $this->insert( array(
                    ...
                    'root_nodeid' =>$rootNodeId,
                    'shared_nodeid' => $sharedNodeId
                    ));
   ...
   * @param string $nodeId The node Id
   */
   function setNodePosition($nodeId){
       //get the parent node
       $parentNodeId=$this->getParentId($nodeId);
   ...
   */
   function reOrderNodes($rootNode =NULL)
   {	
   		$this->next_Node = null;
   		$prev_Node = null;
   		$cnt = 1;
   		...
   		foreach($nodes as $node)
   		{
			...
			$newArr = array('sortindex' => $cnt,							
							'prev_Node' => $this->prevId
							);
   ...
   * @param string $parentID The ParentId
   */
   function _recursiveReOrderNodes($parentId)
   {
   		$next_Node = null;
   		$prev_Node = null;
   		$cnt = 1;
