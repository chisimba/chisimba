...
jQuery.fn.boxy = function(options) {
    options = jQuery.extend({
            ...
            ajaxError: null,
            ajaxComplete: null,
            ajaxSessionExpired: null,
            sessionName: 'PHPSESSID',
            showBoxOnSessionExpiry: true //Weather or not to hide the box when a session has expired
        }, 
            options || {}
    );
        ...
        if (node == 'a') {
            jQuery(this).click(function() {
                ...
                var loadContent = function(after) {
                    if (Boxy.cache[href]) {
                        ...
                        jQuery.ajax({
                            ...
                            beforeSend: function() {
                                //Charl Mert - Chisimba Requirements
                                if (typeof options.ajaxBeforeSend == 'function'){
                            ...
                            complete: function(data) {
                                ...
                                //Checking Server Session State - Hiding boxy if server session has expired
                                var sessionId = data.getResponseHeader(options.sessionName);
                                if (sessionId == null){
                                    if (typeof options.ajaxSessionExpired == 'function'){
                                        options.ajaxSessionExpired(data);
                                    }
                                }

                                if (typeof options.ajaxComplete == 'function'){
                                    options.ajaxComplete(data);
                                }
                            }
                        });
                    }
                ...
                if (options.single && (active = jQuery.data(this, 'active.boxy'))) {
                    if (!options.showBoxOnSessionExpiry){
                        loadContent(function(content) {
                            if (sessionId != null){
                                //Showing Boxy only if session hasn't expired
